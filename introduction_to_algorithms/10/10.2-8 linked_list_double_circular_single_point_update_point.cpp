...
struct List {
    ...
    Node* get_prev(Node *cur, Node *next) {
        return (Node*)((uint64_t)cur->np ^ (uint64_t)next);
    }

    Node* get_next(Node *cur, Node *prev) {
        return (Node*)((uint64_t)cur->np ^ (uint64_t)prev);
    }

    Node* get_np(Node* prev, Node *next) {
        return (Node*)((uint64_t)prev ^ (uint64_t)next);
    }
